...
// Bar entry for gdeyoung.powercore: an icon button that opens Panel.qml.
// Left click opens the panel; right click cycles the extra label mode
// (nothing / percentage / watts / both); middle click cycles the keyboard
// backlight through its steps. No policy logic lives here — everything goes
// through the service.
BarWidget {
  id: root
  ...
  readonly property string barExtra: settingsView.barExtra
  readonly property var shows: Model.barModeShows(barExtra)
  readonly property bool showPercentage: shows.percentage && !vertical && batteryPresent
  readonly property bool showWatts: shows.watts && !vertical
  readonly property bool showGauge: settingsView.barGauge && batteryPresent
  ...
  // Watts come from the service sampler; the widget never reads sysfs.
  ...
  readonly property string wattsText: showWatts && drawWatts > 0 ? Model.formatWatts(drawWatts) : ""

  function cycleBarMode() {
    if (root.service && typeof root.service.saveSettings === "function") {
      var patch = { barExtra: Model.nextBarMode(settingsView.barExtra, batteryPresent) };
      root.service.saveSettings(patch);
    }
  }

  function cycleKbd() {
    if (root.service && root.service.kbdAvailable) {
      var percent = root.service.appliedKbdPercent !== undefined ? root.service.appliedKbdPercent : 0;
      var raw = Model.percentToRaw(percent, root.service.kbdLedMax);
      root.service.setKbdManual(Model.nextKbdStep(raw, root.service.kbdLedMax));
    }
  }
  ...
  BarIconButton {
    ...
    text: {
      var parts = [];
      if (root.showPercentage && root.percentage >= 0)
        parts.push(root.percentage + "%");
      if (root.wattsText !== "")
        parts.push(root.wattsText);
      parts.push(root.icon());
      return parts.join(" ");
    }
    slotSize: Style.bar.iconSlot * ((root.showPercentage ? 1 : 0) + (root.wattsText !== "" ? 1 : 0) + 1)
    // On a desktop the source never changes, so the tooltip names the profile.
    tooltipText: "Power · " + (root.batteryPresent ? Model.sourceLabel(UPower.onBattery ? "battery" : "ac") : (root.service && root.service.activeProfile ? root.service.activeProfile : "Plugged in"))
